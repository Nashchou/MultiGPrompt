...
class downprompt(nn.Module):
    def __init__(self, prompt1, prompt2, prompt3, ft_in, nb_classes, feature, labels):
        ...
        # for x in range(0, 3):
        #     if labels[x].item() == 0:
        #         self.aveemb0 = feature[x]
        #     if labels[x].item() == 1:
        #         self.aveemb1 = feature[x]
        #     if labels[x].item() == 2:
        #         self.aveemb2 = feature[x]
        #     # if labels[x].item() == 3:
        #     #     self.aveemb3 = feature[x]
        #     # if labels[x].item() == 4:
        #     #     self.aveemb4 = feature[x]
        #     # if labels[x].item() == 5:
        #     #     self.aveemb5 = feature[x]
        #     # if labels[x].item() == 6:
        #     #     self.aveemb6 = feature[x]
        # self.ave = torch.stack((self.aveemb0,self.aveemb1,self.aveemb2)).cuda()
        # print("avesize",self.ave.size(),"ave",self.ave)
        # print("feature",feature.shape)
        self.ave = averageemb(labels=self.labels, rawret=feature)
        # emb0 = torch.zeros(1, 1, 5    12).cuda()
...
def averageemb(labels,rawret):
    retlabel = torch.FloatTensor(3,int(rawret.shape[0]/3),int(rawret.shape[1])).cuda()
    cnt1 = 0
    cnt2 = 0
    cnt3 = 0
    cnt4 = 0
